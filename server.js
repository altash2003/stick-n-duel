...
// --- MONGODB CONNECTION ---
// On Railway, this MUST use the MONGO_URL variable.
const dbURI = process.env.MONGO_URL || 'mongodb://127.0.0.1:27017/sticknduel';
mongoose.connect(dbURI)
    .then(() => console.log('✅ Connected to MongoDB'))
    .catch(err => {
        console.error('❌ MongoDB Connection Error:', err);
        // We do not exit the process here so the server stays alive, 
        // but API calls requiring DB will fail until it connects.
    });
...
app.post('/api/register', async (req, res) => {
    ...
    try {
        // --- DEBUG CHECK: Is DB Connected? ---
        if (mongoose.connection.readyState !== 1) {
            console.error("❌ Database is not connected!");
            return res.status(500).json({ error: "Database not connected. Check logs." });
        }
...
io.on('connection', (socket) => {
    ...
    socket.on('place_spectator_bet', async (data) => {
        ...
        } catch (err) { console.error("Spectator Bet Error:", err); }
    });
